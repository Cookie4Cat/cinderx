...
TESTGATE_DIR = REPO_ROOT / "cinderx" / "TestScripts" / "TestScriptsKunpeng"
...
def parse_pytest_summary(log_path: Path) -> dict[str, int] | None:
    ...
    """
    counts = {"passed": 0, "failed": 0, "error": 0, "skipped": 0, "deselected": 0}
    try:
...
def run_job(job: dict[str, Any], run_dir: Path, prelude: str) -> dict[str, Any]:
    ...
    if test_counts:
        parts = []
        for key in ("passed", "failed", "error", "skipped", "deselected"):
            if test_counts[key]:
...
def main(argv: list[str]) -> int:
    ...
    totals = {"passed": 0, "failed": 0, "error": 0, "skipped": 0, "deselected": 0}
