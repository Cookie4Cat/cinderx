...
class ArmRuntimeTests(unittest.TestCase):
    ...
    def test_builtin_id_addaudithook_after_compile_observed(self) -> None:
        code = textwrap.dedent(
            """
            import sys
            import cinderx.jit as jit
            ...
            assert jit.is_jit_compiled(builtin_id)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            script = f"{tmp}/builtin_id_addaudithook_after_compile.py"
            with open(script, "w", encoding="utf-8") as fp:
                fp.write(code)
