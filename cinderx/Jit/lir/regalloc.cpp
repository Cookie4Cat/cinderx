...
void LinearScanAllocator::calculateLiveIntervals() {
  ...
  for (auto iter = basic_blocks.rbegin(); iter != basic_blocks.rend(); ++iter) {
    ...
    for (auto instr_iter = instrs.rbegin(); instr_iter != instrs.rend();
      ...
      // Favor ABI-friendly registers for short call chains on ARM. This is a
      // soft hint: linear-scan can still pick another register when needed.
      //
      // Restrict the hint to a tight pattern to avoid inflating call-heavy
      // kernels:
      //   call -> (single immediate use) -> call(argN = previous result)
      //
      // We currently keep this hint narrow and only handle GP outputs flowing
      // into the next call's first few GP argument positions. Floating-point
      // call ABI assignment is more subtle because FP arguments are packed into
      // a separate register bank, so leave that to a future patch.
      auto get_preferred_call_register =
          [&](const Operand* def_opnd) -> std::optional<PhyLocation> {
        ...
        if (def_opnd->isFp()) {
          if (use_instr->getNumInputs() >= 2 && use_instr->getInput(1) == use_opnd) {
            return PhyLocation{
                arch::reg_double_return_loc.loc, def_opnd->sizeInBits()};
          }
          return std::nullopt;
        }

        size_t max_gp_arg_inputs = std::min<size_t>(4, ARGUMENT_REGS.size());
        for (size_t input_index = 1; input_index <= max_gp_arg_inputs; input_index++) {
          if (use_instr->getNumInputs() > input_index &&
              use_instr->getInput(input_index) == use_opnd) {
            return PhyLocation{
                ARGUMENT_REGS[input_index - 1].loc, def_opnd->sizeInBits()};
          }
        }
      ...
      if (
        ...
        if (auto preferred_reg = get_preferred_call_register(output_opnd)) {
          auto& interval = getInterval(output_opnd);
          interval.allocateTo(*preferred_reg);
        }
      }
#endif
